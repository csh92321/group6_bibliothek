	...
	<div class="wrap">
		...
		<div align="center" style="margin-top: 40px;">
			<table class="aa" border="1" align="center" color="black">
				...
				<c:forEach var="dto" items="${ntAllList }">
					<tr>
						<td>${dto.writeNo}</td>
						<td><a
							href="${contextPath}/ntboard/ntBoardContentView?ntWriteNo=${dto.writeNo}">
								${dto.title }</a></td>
						<td>${dto.admin }</td>
						<td>${dto.saveDate }</td>
					</tr>
			...
			<c:if test="${loginUser eq 'group6' }">
				<button type="button"
					onclick="location.href='${contextPath}/ntboard/ntBoardWriteForm'">새
					글 작성하기</button>
			</c:if>
		</div>
	</div>
